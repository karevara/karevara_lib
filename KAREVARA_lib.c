...
{
	return KAREVARA_Create_OUT( master_secret_string,
						 customer,
						 batch,
						 start_IMSI,
						   quantity,
...
{
	return  KAREVARA_Create_OUT_fm_INP(	  master_secret,
							  inpstr,
							  transport_key_str,
							  trankey_csv_str,
							  outstr,   maxoutlen,
...
int KAREVARA_lib_Create_INP( IN char master_secret[],
						 ...
						 OUT char logstr[], IN uint32_t loglength)
{
 return KAREVARA_Create_INP(   master_secret,
						   customer,
						   batch,
						   start_IMSI,
						   quantity,
...
{
	return KAREVARA_Load_INP(  inpstr,
					  customer,
					  batch,
					  start_IMSI,
					  quantity,
...
int KAREVARA_lib_Create_INP_fm_Inst(KAREVARA_Inst *Main_Inst,
						...
						char logstr[], uint32_t loglength)
{
 return KAREVARA_Create_INP_fm_Inst( Main_Inst,
						  start_IMSI,
						  Type,
						  Profile,
						  Tran_key_num,
...
int KAREVARA_lib_Create_Inst_fm_INP(OUT KAREVARA_Inst *Main_Inst,
								...
								char logstr[], uint32_t loglength)
{
 return KAREVARA_Create_Inst_fm_INP(  Main_Inst,
								  master_secret,
								  inpstr,
								  transport_key_str,
								  trankey_csv_str,
...
{
 return KAREVARA_Create_CSV (   master_secret_string,
						   customer,
						   batch,
						   quantity,
						   format_string,
...
int KAREVARA_lib_Create_CSV_fm_INP(	IN char master_secret[],
							...
							char logstr[], uint32_t loglength)
{
	return  KAREVARA_Create_CSV_fm_INP(	  master_secret,
							  inpstr,
							  transport_key_str,
							  trankey_csv_str,
							  csvstr,   maxcsvlen,
						      unenccsvstr,   maxunenccsvlen,
							  logstr,loglength);
}
...
int KAREVARA_lib_Get_Tran_Key_fm_CSV(char tran_key[],
						...
						char logstr[], uint32_t loglength)
{
 return Get_Tran_Key_fm_CSV( tran_key,
						  tran_key_num,   trancsv,
						 logstr,  loglength);
}
...
int KAREVARA_lib_Run (KAREVARA_Inst *Main_Inst,
		...
		uint32_t loglength)
{
return KAREVARA (Main_Inst,
		res,reslength,resdelim,
		encres,encreslength,encresdelim,
		logstr,loglength);
}
...
int KAREVARA_lib_Create_Inst(char master_secret_string[],
		...
		uint32_t loglength)
{
return KAREVARA_Create_Inst(master_secret_string,
		customer,
		batch,
		quantity,
		format_string,
